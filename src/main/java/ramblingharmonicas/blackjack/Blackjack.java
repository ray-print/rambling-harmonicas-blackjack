...
static State resolveHands(State finishedState, FastShoe myShoe, Rules theRules) {
    ...
    if (!dealNecessary) {
        ...
        if ((myCacheStatus != DealerCache.Status.NO_CACHE)
            ...
            dealerProbabilities = DealerCache.dealerProbabilitiesCache.get(
                    keyForMap);
...
static ArrayList<ArrayList<State>> solveHardPlayersRecursive 
        (Rules theRules, final boolean possibleDealerBJ) 
            ...
            if (pCardTwo.value() >= pCardOne.value()) {
                ...
                for (CardValue dealerCard : CardValue.twoToAce) {
                    ...
                    assert (   (myState.getBestAction() != myState.getSecondBestAction()) 
                            || (myState.playerBJ())) :
                        "Top two preferred actions are the same in this state and Rules"
                        + myState + "\n" + theRules;
...
static ArrayList<Answer> calculateAllSplitValues(Rules theRules,
    ...
    for (CardValue PCard : CardValue.oneToTen) {
        for (CardValue DCard : CardValue.oneToTen) {
            ...
            if (scratch > 50) {
                System.out.println(
                        "Player: " + PCard + ", dealer: " + DCard + ", value: " + scratch);
            }
...
static State PlayerRecursive(final FastShoe myShoe, final State myState,
    ...
    if (myState.numberCardsInHand() == 1) {
        ...
        for (CardValue q : CardValue.oneToTen) {
            probPostSplitDrawResults[i++] = myShoe.playerProbability(
                    dealerHoleCard, dealerCard, q);
        }
        i = 0;
        ...
        if (postSplitDrawResults.size() > 0) {
            ...
            if (myState.numberCardsInHand() != 1) {
                throw new IllegalStateException(
                        "Yay I found an error at ~ postSplitDraw end of PlayerRecursive");
            }
        ...
        if (theRules.isPossible(Action.HIT, myState)) {
            ...
            for (CardValue val : CardValue.oneToTen) {
                if (probHitResults[i] > 0) {
                    ...
                    hitResults[i] = PlayerRecursive(myShoe, hitResults[i],
                            theRules);
                    myShoe.addCard(drawnCard);
                }
                else {
            ...
            for (CardValue val : CardValue.oneToTen) {
                if (probHitResults[i] > 0) {
                    ...
                    if (doubleResults[i].nextHand()) {
                        doubleResults[i] = PlayerRecursive(myShoe,
                                doubleResults[i], theRules);
                    }
                    else {
                        doubleResults[i] = Blackjack.resolveHands(
                                doubleResults[i], myShoe, theRules);
                    }
                    myShoe.addCard(drawnCard);
                }
    ...
    if (possibleDealerBJOnHoleCard) {
        if (dealerHoleCard) {
            dealerBJWithHoleCard = Blackjack.resolveHands(dealerBJWithHoleCard,
                    myShoe, theRules);
            ...
            throw new NoRecommendationException(
                    "Dealer can't have an early BJ in a no-hole game.");
        }
    }
    if (bestAction == null) {
        StringBuilder sb = new StringBuilder();
        sb.append("I have ").append(actionResults.size())
                .append(" elements in action and hitting is").append(
                hitPossible ? "" : "not")
                .append(" possible .").append("\n\n\n").append(
                ". This is my action vector")
    ...
    if (secondBestAction == null) {
        if ((theRules.numPossibleActions(myState, false) != 1) && (myState.numberCardsInHand() < playerMaxHandSize)) { //TODO -- use stringbuilder here
            State.printStateStatus(myState, "Second action doom.");
            DealerCache.printCacheStatus(); //TODO create/use toString for this
            System.out.println(bestAction + " is my preferred action.");
            System.out.println(
                    "I have " + actionResults.size() + " elements in action and hitting is" + (hitPossible ? "" : "not") + " possible .");
            System.out.println("\n\n\n");
            System.out.println("There are " + theRules.numPossibleActions(
                    myState, false) + " actions possible.");
            ...
            for (Action anAction : Action.values()) {
                ...
                if (theRules.isPossible(anAction, myState)) {
                    if (anAction == Action.HIT) {
                        System.out.println(
                                "myState.numberCardsInHand() is " + myState.numberCardsInHand() + " and " + "theRules.getPlayerMaxHandSize() is " + theRules.getPlayerMaxHandSize());
                    }
    ...
    if (!earlySurrenderPossible) {
        ...
        if (earlySurrenderState.getExpectedValue() > bestEV) {
            ...
            if (bestAction == Action.SURRENDER) {
                ...
                myState.setSecondBestAction(secondBestAction);
                throw new NoRecommendationException(myState, theRules, null,
                        "Early surrender is the best option. "
                        + "However, I've already checked for surrender earlier, so I "
                        + "checked surrender options twice, so I shouldn't be here.");
            }
            else {
...
static double[] getBestEVOfSplitStates(FastShoe myShoe, Rules theRules,
    ...
    for (CardValue q : CardValue.oneToTen) {
        if (cardDrawProbs[q.value() - 1] > 0) {
            ...
            assert (myShoe.numberOfCards() == (theRules.getNumberOfDecks() * 52 - 4)
...
static double[] DealerRecursive(final int[] myCards, final FastShoe myDeck,
    ...
    if (cardsInHand >= theRules.getDealerMaxHandSize()) {
        endProbabilities[0] = 1;
        Blackjack.setApproxProbabilities(endProbabilities, myDeck, handValue,
                Utilities.isSoft(myCards, handValue));
        return endProbabilities;
    }
    ...
    for (int k = 0; k < similarStates.size(); k++) {
        if (!discord) {
            if (!bestAction.equals(similarStates.get(k).getBestAction())) {
                throw new NoRecommendationException(
                        "Error in anyDisAgreement here: there was indeed disagreement."
                        + " The best action was thought to be " + bestAction
                        + ", but here is a state that says otherwise: " + similarStates.get(
                        k).toString());
            }
            if (!secondBestAction.equals(
                    similarStates.get(k).getSecondBestAction())) {
                throw new NoRecommendationException(
                        "Error in anyDisAgreement here: there was indeed disagreement."
...
static double splitSolve(Rules theRules, CardValue PCard, CardValue DCard,
    ...
    if ((acePlayer) && !theRules.hitSplitAces()) {
        return noHitSplitAcesSolve(DCard, PCard, possibleDealerBJ, myShoe,
                theRules);
    }
    ...
    for (CardValue firstDrawnCard : CardValue.oneToTen) {
        ...
        for (CardValue secondDrawnCard : CardValue.oneToTen) {
            if ((probNextCard[(firstDrawnCard.value() - 1)] > 0) && (probOfSecondCard[(secondDrawnCard.value() - 1)] > 0)) {
                ...
                if (evNoResplit > 10) {
                    System.err.println(
                            "Strategy.combinedProbSplitApprox:" + "probThisCombo: " + probThisCombo + "; bestEVOfFirstSplitHand[ (firstDrawnCard.value() -1)]: " + bestEVOfFirstSplitHand[(firstDrawnCard.value() - 1)] + "bestEVOfSecondSplitHand[ (secondDrawnCard.value() -1)])" + bestEVOfSecondSplitHand[(secondDrawnCard.value() - 1)]);
                    assert false;
                }
...
static void consolidateIntoTotalDependent(
    ...
    for (int column = 0; column < hardAnswers.get(0).size(); column++) {
        for (int handValue = 4; handValue < 19; handValue++) {
            ...
            Blackjack.solveConsolidationAndReplace(probThisState, sumOfProbs,
                    similarStates);
        }
    }
}
...
private static ArrayList<State> retrieveStatesOfHandValue(final int handValue,
    ...
    for (int j = 0; j < (constantHandValue.size() - 1); j++) {
        if (constantHandValue.get(j).getDealerUpCard().value() == constantHandValue.get(
                j + 1).getDealerUpCard().value()) ;
        else {
            assert false;
        }
        if (constantHandValue.get(j).handTotal() != constantHandValue.get(j + 1).handTotal()) {
            State.printStateStatus(constantHandValue.get(j),
                    "First failing state");
