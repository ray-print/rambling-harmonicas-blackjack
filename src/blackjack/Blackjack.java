 ...
 * Get rid of this file -- move everything to Utilities or Strategy or some other class.
 *
 * @param args
 */
...
/** Call this before trying to access the cache. If the cache has already been initialized,
...
public static void initCache()
{
   ...
   dealerProbabilitiesCache = new HashMap(
        ...
        (cacheStatus == FULL_CACHE) ? INIT_FULL_CACHE_SIZE : (   
...
/**
 * Store values in the cache which go up to this far in the shoe (12 cards drawn)
 * 
 */
 ...
 */
public static int getCacheStatus(){ return cacheStatus;}
...
case SMALL_CACHE:
   ...
   break;
default: throw new IllegalArgumentException("Cache status must be either NO_CACHE, SMALL_CACHE, or FULL_CACHE");
}  
}
...
/** Given a finished State (a player is done deciding what to do; all hands are finished),
 * this tells you whether or not the dealer needs to deal. It also may change the current
 * player hand (State.currentHand). Throws an IllegalStateException if not all hands are done or 
 * if there is 
 * a logic error.
 * 
 ...
 *  * After this function, if a deal was needed, display the dealer's down card, or draw one 
 * if he has no hole card. 
 * 
 * If the dealer does have BJ (it's a no hole card game), and a deal was necessary,
 * tell State that fact. (If a deal was NOT necessary, do NOT tell State that, just call 
 * calculateEV(theRules) )
 * 
 * If a deal is needed, then deal the dealer out; then call  
 * State.calculateEV(dealerProbabilities, theRules), where dealerProbabilites is the 
...
public static boolean dealNecessary (State finishedState, Rules theRules)
{
           ...
           ) 
   {
      ...
      {      dealNecessary = false; flag = false;
      }
...
/** Creates a key for the dealer probability cache based on the shoe, rules, and dealer up card.
 * 
 * 
 ...
 * Hit/stand on 17
 * The shoe contents
 * The presence or absence of a dealer hole card, but only if the dealer up card is an ace or ten.
 * Accuracy of the rules
 * 
 * @param myShoe The current shoe
 * @param dealerCardIndex This is in the INDEX of a standard dealer card array. 0 = Ace. That cost hours.
 * @param theRules The current rules.
 * @return 
 */
static String getKeyForMap(FastShoe myShoe, final int dealerCardIndex, Rules theRules)
{ StringBuilder building = new StringBuilder();
if (theRules.hitOn17() == true)
   building.append("H");
else building.append("S");
 ...
 *
 *
 * This is a wrapper function for State.calculateEV. This is the function that creates
...
System.out.print("\t");
    ...
    * blackjack.
    */
   static ArrayList<ArrayList<State>> solveHardPlayersRecursive(Rules theRules, final boolean possibleDealerBJ)
   {
         ...
         for (CardValue pCardTwo : Blackjack.twoToTen)
         {
            if (pCardTwo.value() >= pCardOne.value())
            {
               ...
               for (CardValue dealerCard : Blackjack.twoToAce)
               {
                  ...
                  if (!possibleDealerBJ) duh.setDealerBlackjack(false);
    ...
    * @return
    */
   static ArrayList<Answer> calculateAllSplitValues(Rules theRules, ArrayList<ArrayList<State>> hardAnswers, ArrayList<ArrayList<State>> softAnswers, final boolean possibleDealerBJ) throws NoRecommendationException
   {
      double scratch;
   ...
   /** I make a million calls to .size; to make this function faster, avoid those calls.
    * How can calling a simple getter take 8 % of the whole program time??? Makes no sense.
    * To make this faster, I need to make State more lightweight. If I don't intend to use the
    ...
    */
   static State PlayerRecursive(final FastShoe myShoe, final State myState, final Rules theRules) throws NoRecommendationException
   {
      int i;
      ...
      if (myState.isBust())
      {
         if (myState.nextHand()) return PlayerRecursive(myShoe, myState, theRules);
         else return Blackjack.resolveHands(myState, myShoe, theRules);
      }
      ...
      if (((dealerCard.value() == CardValue.TEN.value()) || (dealerCard.value() == CardValue.ACE.value())) && (!myState.dealerBlackJackChecked()))
      {
         ...
         {
            if (Strategy.insuranceGoodIdea(myShoe, theRules, myState)) myState.action(Action.INSURANCE);
         }
         if (dealerHoleCard)
         {
      ...
      for (i = 0; i < actionResults.size(); i++)
      {
         ...
         else if (actionResults.get(i).getExpectedValue() > secondBestEV)
         {
            ...
            else
            {
               ...
               if (actionResults.get(i).lastAction() == null) assert false;
            }
         }
      }
      ...
      if (secondBestAction == null)
      {
         if ((theRules.numPossibleActions(myState, false) != 1) && (myState.numberCardsInHand() < playerMaxHandSize))
         {
            ...
            for (Action anAction : Action.values())
            {
               ...
               {
                  if (anAction == Action.HIT) System.out.println("myState.numberCardsInHand() is " + myState.numberCardsInHand() + " and " + "theRules.getPlayerMaxHandSize() is " + theRules.getPlayerMaxHandSize());
                  if ((anAction == Action.HIT) && (myState.numberCardsInHand() >= theRules.getPlayerMaxHandSize())) ;
                  else System.out.println(anAction + " is possible.");
               }
            }
            try
            {
    ...
    * @return
    */
   static ArrayList<ArrayList<State>> solveSoftPlayersRecursive(Rules theRules, final boolean possibleDealerBJ) throws NoRecommendationException
   {
      final FastShoe myShoe = new FastShoe(theRules.getNumberOfDecks());
      ArrayList<Card> myCards = new ArrayList<Card>();
      final int actualSplitsAllowed = theRules.getMaxNumberSplitHands();
    ...
    *
    *
    * @param cardDrawProbs An array which represents the chances of drawing that card index
    * @param myShoe The current shoe
    ...
    */
   static double[] getBestEVOfSplitStates(FastShoe myShoe, Rules theRules, boolean possibleDealerBJ, CardValue PCard, CardValue DCard) throws NoRecommendationException
   {
      ...
      for (CardValue q : Blackjack.oneToTen)
      {
         if (cardDrawProbs[q.value() - 1] > 0)
         {
            ...
            if (!possibleDealerBJ) scratch.setDealerBlackjack(false);
            if (theRules.dealerHoleCard() == true) scratch.setDealerBlackjack(false);
            scratch = PlayerRecursive(myShoe, scratch, theRules);
            myCards.clear();
            myShoe.addCard(q);
            ...
            else bestEVOfStates[i] = scratch.getExpectedValue();
         }
         else bestEVOfStates[i] = -500000;
         i++;
      }
    ...
    */
   static double[] fastDealerRecursive(final int[] myCards, final FastShoe myDeck, final Rules theRules)
   {
    ...
    *
    * @param someStates
    * @return True if some states are advising a different first or second action than
   ...
   static boolean anyDisagreementHere(ArrayList<State> someStates) throws NoRecommendationException
   {
      ...
      {
         if (someStates.get(i).getFirstCardValue() == someStates.get(i).getSecondCardValue()) throw new NoRecommendationException();
   ...
   /**
    * Find if there is any disagreement as to what to do; if so, it gets ugly really fast.
    ...
    *  --If an action is not listed as being the top 2 actions by a state, I will assume that its
   ...
   //based on their relative probability  (Prob of this hand)/(Prob of all hands of this
    ...
    * @param similarStates
    */
   static void solveConsolidationAndReplace(double[] probThisState, double sumOfProbs, ArrayList<State> similarStates) throws NoRecommendationException
   {
      assert (similarStates.size() == probThisState.length);
      if (similarStates.size() <= 1) return;
      ...
      else
      {
         ...
         for (Action anAction : possibleActions)
         {
            ...
            {
               if (similarStates.get(i).getPreferredAction() == anAction) score[j] += similarStates.get(i).getExpectedValue() * probThisState[i] / sumOfProbs;
               else if (similarStates.get(i).getSecondBestAction() == anAction) score[j] += similarStates.get(i).getSecondBestEV() * probThisState[i] / sumOfProbs;
               else score[j] += (similarStates.get(i).getSecondBestEV() - 0.1) * probThisState[i] / sumOfProbs;
            }
            j++;
         }
         int bestIndex = -1;
   ...
   //Cripes.
   /** Factors in the chance of the dealer having blackjack, including insurance in a hole card game.
    ...
    *
    */
   private static double splitApproxIncludingDealerBJ(Rules theRules, final double probDealerBJ, final double originalSplitEV, CardValue PCard, CardValue DCard)
   {
      ...
      else return originalSplitEV;
   }

   private static double noHitSplitAcesSolve(CardValue DCard, CardValue PCard, boolean possibleDealerBJ, FastShoe myShoe, Rules theRules) throws NoRecommendationException
   {
      State scratch = new State(PCard, PCard, DCard);
      if ((!possibleDealerBJ) && ((DCard == CardValue.TEN) || (DCard == CardValue.ACE))) scratch.setDealerBlackjack(false);
      scratch = Blackjack.PlayerRecursive(myShoe, scratch, theRules);
      if (scratch.getPreferredAction() == Action.SPLIT) return scratch.getExpectedValue();
      else if (scratch.getSecondBestAction() == Action.SPLIT) return scratch.getSecondBestEV();
      else
      {
   ...
   /**
    * Tested under 1 rule set; mostly works, but off by a large margin in certain rare
    * circumstances (when only one deck is being used). Don't know why.
    * Note the special treatment of dealer blackjacks. In a hole card game, they are impossible
    * on split hands, since you'd never split. However, they still must be factored into this
    * decision, since they are factored into the other decisions. So: completely discount
    ...
    */
   static double splitSolve(Rules theRules, CardValue PCard, CardValue DCard, final boolean possibleDealerBJ) throws NoRecommendationException
   {
      final long originalRulesHash;
      if (Blackjack.debug()) originalRulesHash = theRules.myHashKey();
      else originalRulesHash = -1;
      ...
      if (possibleDealerBJ)
      {
         if (DCard == CardValue.TEN) probDealerBJ = myShoe.fastProbabilityOf(CardValue.ACE);
         else if (DCard == CardValue.ACE) probDealerBJ = myShoe.fastProbabilityOf(CardValue.TEN);
         else probDealerBJ = -1000;
      }
      else probDealerBJ = -1000;
      if ((acePlayer) && !theRules.hitSplitAces()) return noHitSplitAcesSolve(DCard, PCard, possibleDealerBJ, myShoe, theRules);
      ...
      if (Blackjack.debug()) theRules.setBlackjackPayback(9.98); 
      else theRules.setBlackjackPayback(1);
      ...
      if ((maxNumberSplitHands > 1) && !(acePlayer && (theRules.getNumResplitAces() == 0))) resplitPossible = true;
      else resplitPossible = false;
   ...
   static double combinedProbSplitApprox(final double[] bestEVOfFirstSplitHand, FastShoe myShoe, CardValue PCard, final boolean resplitAllowed, Rules theRules, boolean dealerBJPossible, CardValue DCard, final boolean acePlayer) throws NoRecommendationException
   {
      ...
      for (int kk = 0; kk < fastBestEVOfThirdSplitHand.length; kk++) if (fastProbCardsOnResplit[kk] > 0) fastEVOfResplit += fastBestEVOfThirdSplitHand[kk] * fastProbCardsOnResplit[kk];
      myShoe.addCard(PCard);
      for (CardValue firstDrawnCard : Blackjack.oneToTen)
      {
         ...
         for (CardValue secondDrawnCard : Blackjack.oneToTen)
         {
            if ((probNextCard[(firstDrawnCard.value() - 1)] > 0) && (probOfSecondCard[(secondDrawnCard.value() - 1)] > 0))
            {
               ...
               if (Blackjack.debug()) sumTest += probThisCombo;
               ...
               else evResplit += probThisCombo * (bestEVOfFirstSplitHand[(firstDrawnCard.value() - 1)] + bestEVOfSecondSplitHand[(secondDrawnCard.value() - 1)]);
            }
         }
         myShoe.addCard(firstDrawnCard);
      }
      if (Blackjack.debug()) assert ((sumTest < 1.001) && (sumTest > 0.999));
      if (!resplitAllowed) return evNoResplit;
      if (evResplit > evNoResplit) return evResplit;
      else return evNoResplit;
   }
    ...
    */
 static void consolidateIntoTotalDependent(ArrayList<ArrayList<State>> hardAnswers, Rules theRules) throws NoRecommendationException
   {
      ...
      for (int column = 0; column < hardAnswers.get(0).size(); column++) for (int handValue = 4; handValue < 19; handValue++)
         {
    ...
    * hands and I believe that usually the player has the option to split with them.
    ...
    * @return
    */
   private static ArrayList<State> retrieveStatesOfHandValue(final int handValue, ArrayList<State> theStates)
   {
      ArrayList<State> constantHandValue = new ArrayList<State>();
      for (int i = 0; i < theStates.size(); i++)
      {
         if ((theStates.get(i).handTotal() == handValue) && (theStates.get(i).getFirstCardValue() != theStates.get(i).getSecondCardValue())) constantHandValue.add(theStates.get(i));
      }
