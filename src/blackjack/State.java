 ...
 * When the dealer's hand has been solved, call State.calculateEV with the correct
 * information, and it will set the expected value for this State. Alternatively,
 ...
 * State to 3. If false, it sets the max number of hands to 20. Note this is static.
 * It should be set 
 */
...
private static String getStateString(State myState)
{
     StringBuilder mofo = new StringBuilder();
      ...
      if ((myState.getFirstCardValue() == myState.getSecondCardValue()) && myState.numberCardsInHand() == 2) 
 ...
 * index of myHands and of playerActions. In other words, it's 0 when there are no splits;
...
{   ////I assume this is called by some other function which does the
   //actual work
 ...
 * In some circumstances -- player blackjack -- the only option will be to stand.
 ...
 * 
 * @return 
 * @throws IllegalStateException if the dealer blackjack has not been checked. Always
 ...
 * 
 * 
 * @return true if, in the current game state, the dealer has already checked for blackjack.
 * False otherwise.
 */
...
{//return new ArrayList<ArrayList<Card>>(myHands); does NOT work. Creates a shallow clone.
 ...
 * @param dealerCard 
 */
public State(final Card firstPlayerCard, final Card secondPlayerCard, final Card dealerCard)
...
/**
 *
 * Throws exception if the initial hand size is not equal to 2 or if the arguments are null.
 * Deprecated because other functions do not need to know -- and it's annoying for them to have
...
/** Returns true if the player's current hand total is over 21. Returns false otherwise.
 * 
 */
...
/** Returns true if the player's hand total is more than 21.(Aces count as one here.)
 ...
 * 
 * Returns hand value of current hand.. Aces count 11 if they can, 1 if they can't.
...
/** Takes the specified action. May lead to strange results if the action is impossible.
 * Before this is called, it should be verified that the action is indeed possible --
 * call Rules.isPossible(Action, State) to check.
 * 
 * Upon action completion, this will correctly marks the hand as done or not done.
 *
 * Note that split actions stack. Meaning, if I have a hand, and I split it, and I split it again,
 * the first two actions in that hand listed as splits.
 *

 * @throws IllegalStateException If being told to take insurance, this will throw an exception if:
 * 
...
{ //Possible: Split; surrender; Stand; possibly insurance.
...
/** Makes the players hit or double by adding that action and drawing the specified card.
 * @throws IllegalArgumentException if it's called with an Action other than
 * double or hit.
 * @throws IllegalStateException if it's called when the current hand is marked as being done.
 * This task (checking if the hand is done) should be done by whichever function called it.
 *
 * Tested several times with a variety of cards. Seems to throw exceptions as listed.
 * Hit and double seem to correctly mark the hand as done, and put the card in hand, and
 ...
 * This task (checking if the hand is done) should be done by whichever function called it.
 *
 * This function should really just call action(Action, Card) to avoid code duplication.
 *
 * @param myAction Adds this action to the action list of the current hand.
 * @param myCard Adds this card to the current hand.
 *
...
/***
 *  Note that the second and subsequent hands do NOT have a split listed as the first action.
...
/**
 * @return The total number of player hands, minus one. For example, if the player has one hand,
 ...
 * for all hands. 0 indicates the player neither lost nor gained money; 1.5 indicates
 * that the player earned 1.5 times his original bet amount; -2 indicates that the player
 * lost 2 times his original bet amount. To get a per-hand breakdown, use getHandResult
...
/** The expected value of this hand when the course of action followed by SecondBestAction
 * has been taken.
 * 
 * @return 
 */
 ...
 * This function should be used to figure out what is happening and then plan for it.
 * When a hand is done, nextHand() should be called, in case the player has split.
 *
 */
boolean isHandDone()
{
 ...
 * action was taken.
 * @throws IllegalArgumentException if handIndex is negative, or refers to a hand that doesn't
 ...
 * @return True if the first card is equal to the passed CardValue, false otherwise.
 *
 */
...
/** Seems to perform as expected. Should only be called after a split hand has done its
 * last action.
 *
 *
 *
 ...
 * Do NOT call me in a no-hole-card game with dealer blackjack., unless the blackjack is irrelevant
 ...
 * Converts handTotal to dealer Probability array then passes it on to calculation function.
 * UNTESTED
 * If the player has a blackjack, and the dealer does not, this function
 * passes control on to calculateEV(theRules), since the dealer's hand total is not
 ...
 */
public void calculateEV(final int dealerHandTotal, Rules theRules, boolean dealerBlackjack) 
{
   ...
   {  case 17: dealerProbabilities[2] = 1; break;
      case 18: dealerProbabilities[3] = 1; break;
      case 19: dealerProbabilities[4] = 1; break;
      case 20: dealerProbabilities[5] = 1; break;
      case 21: dealerProbabilities[6] = 1; break;
      default:
         if (!playerBJ())
 ...
 * State.BLACKJACK is only returned if you have a blackjack and the dealer does not. If the
 * dealer does have a blackjack, this would return State.PUSH if you as well have a blackjack.
 * 
 * This should be changed if Australian Late Surrender is implemented (it'd have to take
 ...
 * I'm assuming you can win an insurance bet even if you later go bust. I don't know the truth.
 Dealer array:
 an array of doubles:
...
/**
 *
 * @return True if your current hand is worth 21; it has two cards in it; and you only have one hand.
 * Hopefully, none of these facts will change in the future; I need to hardwire that inability
 * to change into the program somehow.
 */
public boolean playerBJ()
{ //Your first hand has two cards
...
/** Should be a duplicate of the hash function belonging to the private Answer Class.
 * Good rewrite: Make this a utilities function, not belonging to any object.
 * But it can be called by this function (after the error checking) and the Answer function.
 * @return The answer hash corresponding to this state; will be a split answer hash if TRUE
 ...
 *
 * TOTALLY UNTESTED. Especially check that it gives the right hand value for a variety of hands.
 */
