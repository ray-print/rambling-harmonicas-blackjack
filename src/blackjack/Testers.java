...
Answer hard, split;
      ...
      { //As expected; splitting is not a good solution.
      }
...
//      http://wizardofodds.com/games/blackjack/appendix/3b/
     ...
     assert (aStrategy.findBestAction(someState) == Action.DOUBLE);      //-> Hit comp-dep
          if (verbosity)
      {  //haven't loaded the rules into the strategy before the first assert
...
/**
 *  Tests the given Strategy to check that it's truly total-dependent and none of it
 * is composition-dependent. Used to test that the conversion from composition-dependent
 * to total-dependent works.
 * @param verbosity
 * @param aStrategy 
 */
private static void testTotalConsolidatedForConsolidation(boolean verbosity, Strategy aStrategy)
 ...
 */
private static void setActionsForHandTotals (boolean lowHands, Action [] bestAction,
 ...
 * Essentially a wrapper function for testOneSolvedStrategy and testTotalConsolidatedForConsolidation
 * 
 * @param aStrategy 
 */
 ...
 *  Tests the given strategy to make sure that it obeys common sense blackjack rules:
 ...
 * If you have hard 14 and over with a dealer 2-6 up, you should always stand or split.
 * 
 * @param dealerCard
 ...
 * 
 */
private static void testOneSolvedStrategy(CardValue dealerCard,CardValue firstPlayerCard,CardValue secondPlayerCard,
...
{ 
   ...
   if ( 
         (dealerCard.value() < 8) && (dealerCard.value() != CardValue.ACE.value()) &&
   ...
   { isSoft = true; handTotal = firstPlayerCard.value() + secondPlayerCard.value() + 10;
   }
   else { isSoft = false; handTotal = firstPlayerCard.value() + secondPlayerCard.value(); }
 ...
 * 
 */
public static void viewRawSplitEV(Rules theRules, ArrayList<ArrayList<State>> solvedHard,
...
private static void shoeCopyConstructorTest(int numberDecks) throws ShuffleNeededException
{
 ...
 assert ( firstShoe.fastProbabilityOf(CardValue.NINE) <
   ...
   if (numberDecks != 1)
      assert ( secondShoe.fastProbabilityOf(CardValue.NINE) >
          ( (4D * numDecks - 4D) / (52D*numDecks - 4D)  - Blackjack.EPSILON) );
   else assert ( secondShoe.fastProbabilityOf(CardValue.NINE) < 0);
}
...
/**
 *  Currently just tests the copy constructor. Note that the copy constructor is also
...
public static void dealerClassTest()
{
   ...
   someCards[1] = 1;
    try { Utilities.retrieveSingleCard(someCards); 
...
final double [] calculatedResults = Blackjack.fastDealerRecursive
        (handArray, new FastShoe(myShoe), theRules);
...
{  myShoe = new Shoe(theRules.getNumberOfDecks());
   ...
   if (handTotal > 21)
      ...
      manualResults[1]++;
   else manualResults[(handTotal - 15)]++;
...
{ //System.out.print(manualResults[j] + "   ");
...
static State testPlayerRecursive(final Card DCard, final Card firstPCard, 
        final Card secondPCard, final Rules theRules, FastShoe myShoe, final boolean dealerBJPossible)
{ ArrayList<Card> myCards = new ArrayList<Card>();
myCards.add(firstPCard); myCards.add(secondPCard);
State myState = new State(myCards, DCard);
if (!dealerBJPossible)
   myState.setDealerBlackjack(false); try {
 ...
 *
 */
static void printStrategy(ArrayList<ArrayList<State>> solvedStates, String message, final boolean printSecondBest)
{
...
/**testDrawAppropriateFunctions -- some parts are mothballed (2-6, 7-A, + ?1 more)
 * 
 * 
...
{  testFastShoeKey();
   ...
   { if (CardValue.TEN == val)
     ...
     if  ( Math.abs(myShoe.probTheseThreeInOrder(val, val, val)  - 
             ...
             < 0.001   )  
        ;
     else throw new RuntimeException("Prob these three failed.");
   } 
   for (int i = 0; i < 8; i++)
...
/**
 * Tests the rules hash for all values. See rules hash key function for more information.
 * 
 *
 *
 ...
 *  Since the specific EVs are not stored in files, forceSolve must be set to true 
...
/** Provides simple testing of split answers. Does no testing on early surrender cases.
 *
 * Add assert back in after mystery bug found on 8 8 A that thinks splitting is a bad
 * idea.
 * 
 ...
 */
 @Deprecated
static public void simplePrintAnswer(ArrayList<Answer> stupidAnswers, Rules theRules, final boolean verbosity)
{
      CardValue firstCard;
 ...
 * @param theRules
 * */
static public void testDirectSplitAlgorithm
    (CardValue playerCard, CardValue dealCard, Rules theRules)
        {
           try {
 ...
 * 
 */
    static public void viewRawStrategy (Rules theRules, String accuracyLevel, final boolean viewSecondBest)
    {
       try {
   ...
   /**
    *  This function relies on the exact values of the byte when CardValues are converted
    * to bytes. If those values change this function will probably throw an exception.
    * 
    */
   public static void testAnswerHash()
   {
      ...
      for (first = 1; first < 11; first++)
         for (second = first; second < 11; second++) 
            //The order of first and second cards doesn't matter
 ...
 */
   ...
   private static void wrapperCheckTotalEV(double expectedEV, Rules theRules, boolean verbosity)
   {
     wrapperCheckTotalEV(expectedEV, theRules, verbosity, Strategy.Skill.COMP_DEP);
   }
           
   private static void wrapperCheckTotalEV(double expectedEV, Rules theRules, boolean verbosity, Strategy.Skill myLevel)
   {
   ...
   private static boolean checkTotalEV(double expectedHouseEdge, double totalEV, Rules theRules, Strategy myStrats, boolean verbosity)
   {
      final boolean percentErrorPass;
   ...
   static public void doAll(boolean verbosity, boolean savingStatus)
   {
      ...
      if (verbosity) System.out.println("All single deck tests passed.");
      Blackjack.clearCache();
      
      
      totalEVTwoDeckHit17HoleCard(verbosity);
      if (verbosity) System.out.println("All double deck tests passed.");
      Blackjack.clearCache();
      
      totalEVFourDeckStand17NoHole(verbosity);
      totalEVFourDeckStand17HoleCard(verbosity);
            if (verbosity) System.out.println("All four deck tests passed.");
      Blackjack.clearCache();
      

      
      totalEVEightDeckHit17HoleCard(verbosity);
      if (verbosity) System.out.println("All eight deck tests passed.");
      Blackjack.clearCache();
      ...
      if (verbosity) System.out.println("All total EV tests passed.");
   }
