 ...
 * dealer recursive function. This is unnecessarily confusing. If that is ever changed
...
s.append("This fast shoe contains a total of ").append
        (this.numberOfCards()).append(" cards: ").append(ln);
 ...
 *
 * @param cardIndex
 * @return A negative number if there are none of those cards left in the shoe or if
 * the shoe is empty
 *
 */
 ...
 * cards with the excluded CardValue cannot be drawn. Returns a negative value if the CardValue 
...
/** cardValueCache[0] = -50000
 *  and cardValueCache[1] holds the number of Aces. However, the indexing system is different
 ...
 *
 * @param value
 * @return 0 if value is 0 or less. Otherwise, the chances of getting this value or less,
 ...
 */
public double probTheseThreeInOrder(CardValue firstCard, CardValue secondCard, CardValue dealerCard)
{
...
/** This function is not directly tested but is a wrapper for fastProbablityOf(int),
 * which is well tested.
 * Speed ideas -- first, inline this to make it faster, second, don't double allocate
 ...
 * the dealer has a ten or ace up and a hole card, she has NOT drawn, respectively, 
 * an ace or a ten, since in that case he'd have already been blackjack and game over.
 */
...
//ERROR CHECKING
 ...
 double sum =0;
 for (i= 0; i <probabilities.length; i++)
 { if (probabilities[i] > 0) sum += probabilities[i];
 ...
 * @return 
 */
double [] getDealerProbabilities(int cardsInHand,boolean dealerHoleCard,int [] myCards)
{
