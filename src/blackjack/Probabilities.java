...
public class Probabilities {
     ...
     * without going bust or having hard 17 / soft 18 or better, given the dealer up card.
     */
    static double largeHandProbability
            (CardValue dealerCard, int numberOfCards, final FastShoe myDeck, final Rules theRules)
    {int cardsInHand = 1;
    ...
    private static double DealerProbabilityRecursive
...
i = 0; 
for (CardValue q : CardValue.values() ) { oneToTen[i++]= q; if (i == 10) break; }

if (    (myCards.size() == 1) && (theRules.dealerHoleCard() == true) )
   { //System.out.println("YO!!!");
       if (myCards.get(0).getCardValue() == CardValue.ACE)
       {   i = 0;     Probabilities[9] = 0; tenDraw = false;
     ...
     //NO ACES POSSIBLE
        ...
        { i=0;  Probabilities[0] = 0;  aceDraw = false;
       // System.out.println("Allo");
       for (CardValue val : oneToTen )
          {
              if (val != CardValue.ACE)
              {//System.out.println("val is " + val.toString() + " and the probability is " +
...
for (CardValue val : oneToTen) {
   if (  (val == CardValue.ACE) && (aceDraw == false) ){ i++; continue;}
   if (  (val.value() == 10) && (tenDraw == false) ) {i++; continue; }
...
return totalDealerProbability; // when would I ever get here?? Oh, at the very, very end.
    ...
    *
    */
   static double DealerRecursiveSandBox(final ArrayList<Card> myCards, final FastShoe myDeck, final Rules theRules, double probHere)
   {
