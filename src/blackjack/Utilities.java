...
public class Utilities {
   ...
   public static String stackTraceToString(Throwable t)
   {
     ...
     * Not using this function, because it duplicates handTotal and I need to save time.
     * 
     * There's a function in State that does the same as this function. This one or the
    ...
    if (myCards.length != 10) throw new RuntimeException("Dealer.handValue error: Array of wrong length.");
     ...
     * @return True if myCards contains a card of thisValue; false otherwise. Tens are all the same.
     */
...
{ 
   ...
   static float[] doublesToFloat(double[] array)
   {
      ...
      for (int i = 0; i < array.length; i++) inFloatForm[i] = (float) array[i];
      return inFloatForm;
   }
    ...
    * @return
    */
   static double averageBestEV(ArrayList<State> someStates, double[] probThisState, double sumOfProbs)
   {
      double total = 0;
      for (int i = 0; i < someStates.size(); i++) total += someStates.get(i).getExpectedValue() * probThisState[i] / sumOfProbs;
      return total;
   }
    ...
    * thing to do?
    */
  static Answer splitEVtoAnswer(final double splitEV, CardValue PCard, CardValue DCard, ArrayList<ArrayList<State>> hardAnswers, ArrayList<ArrayList<State>> softAnswers)
   {
      State baseState;
   ...
   static double combinedStatesEV(State[] possibleStates, double[] probabilities) throws NoRecommendationException
   {
      ...
      {
         double sum = 0;
         for (int i = 0; i < probabilities.length; i++) if (probabilities[i] > 0) sum += probabilities[i];
      ...
      for (int i = 0; i < possibleStates.length; i++)
      {
         ...
         {
            if (possibleStates[i].getExpectedValue() < -10) throw new NoRecommendationException("IllegalStateException:Error discovered in combinedStatesEV");
    ...
    * an impossible state.
    */
   static double combinedStatesEV(ArrayList<State> possibleStates, double[] probabilities)
   {
      ...
      for (int i = 0; i < possibleStates.size(); i++)
      {
         if (probabilities[i] > 0)
         {
            if (possibleStates.get(i).getExpectedValue() < -10) throw new IllegalStateException("Error discovered in combinedStatesEV");
   ...
   {
      if ((array == null) || (array.length == 0)) throw new NoSuchElementException();
      int q = array[0];
      for (int i = 1; i < array.length; i++) if (array[i] > q) q = array[i];
      return q;
   }
   ...
   static double[] floatsToDouble(float[] array)
   {
      ...
      for (int i = 0; i < array.length; i++) inDoubleForm[i] = (double) array[i];
      return inDoubleForm;
   }

   static double averageSecondBestEV(ArrayList<State> someStates, double[] probThisState, double sumOfProbs)
   {
      double total = 0;
      for (int i = 0; i < someStates.size(); i++) total += someStates.get(i).getSecondBestEV() * probThisState[i] / sumOfProbs;
      return total;
   }
    ...
    * the passed player firstCard and secondCard, and dealerCard (or a deep clone of the
    ...
    *
    */
 static State findStartHand(ArrayList<ArrayList<State>> solvedStates, CardValue firstCard, CardValue secondCard, CardValue dealerCard, boolean deepClone)
   {
...
/** Draw all dealer cards randomly based on the current rule set. If the Rules state that
 * the dealer has a hole card, that hole card must be added back in to this array before calling
 * this function, otherwise, an exception should be thrown. The reason is that the hole card
 * must have been pulled from the deck before the player drew his hand -- otherwise, the
 ...
 */
public static  ArrayList<Card> getDealerHand(Rules theRules, ArrayList<Card> dealerCards, Shoe myShoe)
{
   ...
   {  if ( Utilities.isSoft(dealerCards) ) // and it's soft
         { if (theRules.hitOn17() ) ; //and you hit on soft 17s, then keep going
             else return dealerCards; //and you stand on soft 17s, then return
         }
      else return dealerCards; //17 and your hand is hard ->return
   }
   //I should hit now.
 ...
 * This should only be used with no-hole games, because for hole card games you should already
 * have two cards in hand (one is the hidden hole card) before finishing the hand.
 * 
 * @param theRules
 * @param dealerUpCard
 * @return Finished dealer hand.
 */
 public static ArrayList<Card> getDealerHand(Rules theRules, Card dealerUpCard, Shoe myShoe)
