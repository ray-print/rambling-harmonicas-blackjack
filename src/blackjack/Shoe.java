 ...
 *
 * 
 * Future development: I can draw a random card; I can reset it to whatever I want.
 */
public class Shoe implements VagueShoe {
     ...
     * Alternately, create another thread whose sole job is to generate random numbers,
   ...
   {
       if (numberOfDecks <= 0) throw new IllegalArgumentException();
    ...
    * @param thisCard
    * @return The probability of drawing this CardValue. Returns a negative number if no cards
  ...
  //Java requires explicit upcasting.
    ...
    * @exception throws an IllegalArgumentException if there are no cards in the shoe or
   ...
   {if ( myCards.isEmpty()) throw 
   new ShuffleNeededException("Shoe.drawSpecific(CardValue) called with no cards left in the shoe.");
   ...
   public Card drawRandom () 
           { //OK. This exception is way too much of a pain to deal with. 
       if (totalCards == 0) 
       {
...
{ int errorCondition = 0;
  ...
  boolean flag = true;
   ...
   /** 
   ** Draws the first card for the player if player is set to true, otherwise draws the dealer card.
 ...
 * 
 * @param player True if you're drawing the first player card, false if you're drawing the dealer card
...
public Card drawAppropriate(DrawMode myMode, boolean player, Rules theRules) throws ShuffleNeededException
      ...
      if (!player)
      {
         ...
         return drawRandom(); 
          ...
          throw new UnsupportedOperationException
                  ("This DrawMode not supported for the dealer: " + myMode.toString());
         }
      }
   ...
   /** Draws a card between the low card value (Ace = 1) and the high card value (10),
    ...
    * 
    */
private Card drawBetween (final int lowCardValue, final int highCardValue) throws ShuffleNeededException
{ //Check possibility
   if (lowCardValue > highCardValue)
    ...
    if (pulledCard == null) throw new NullPointerException();
  ...
  public Card drawAppropriate(DrawMode myMode, Card previouslyDrawnCard) throws ShuffleNeededException
   {
    ...
    switch (myMode)
      ...
      case ALL_SOFT:
         ...
         else return drawSpecific(CardValue.ACE);
      case ALL_SPLITS:
   ...
   /** Draws first card in shoe of specified value. About seven times faster than drawSpecific.
    * 
    * 
    * @param thisCard
    * @return 
   ...
   {if ( myCards.isEmpty()) throw 
   new RuntimeException("Shoe.fastDrawSpecific(CardValue) called with no cards left in the shoe.");
       //locate all cards that match that value, pick one of them, if one of them exists.
              int i;
        //      ArrayList<Integer> locations = new ArrayList<Integer>();
 ...
 */
   ...
   * 
    ...
    * @return The probability of drawing the specified CardValue from the Shoe, given that all
    * cards with the excluded CardValue cannot be drawn. Returns -1 if the CardValue is not present
    * in the shoe or if the Shoe is empty.
    * @throws IllegalArgumentException if the two passed CardValues are the same.
    *
   public double probabilityOfExcluding(final CardValue thisCard, final CardValue excluded)
   {if ( myCards.isEmpty())
       return -1;
